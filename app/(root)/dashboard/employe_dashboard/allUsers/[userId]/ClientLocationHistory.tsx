...
export default function ClientLocationHistory({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    if (userId) {
      const fetchLocations = async () => {
        try {
          setLoading(true);
          const res = await api.get(`https://ems-api-theta-blush.vercel.app/api/location?userId=${userId}`);
          if (!res.data) {
